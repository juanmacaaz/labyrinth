...
HudSpace::HudSpace(Engine* engine) : Space(engine) 
{
	...
	for (int i = 0; i < 6; i++) {
		...
		for (int j = 0; j < 10; j++) {
			nombre_calquiera.push_back(new Entitie(this, "num_"+std::to_string(j), Block::WHITE, "basic-nolight", positions[i], scale[i]));
		}
		vectors_numeros.push_back(nombre_calquiera);
	}

	//YOU SCORE
	hudElements.push_back(new Entitie(this, "key_model", Block::WHITE, "basic-nolight", Vector3(0.15, 0.048, -0.065), 0.08));
	hudElements.push_back(new Entitie(this, "you_text", Block::WHITE, "basic-nolight", Vector3(0.15, 0.058, -0.08), 0.002));

	//ENEMY SCORE	
	hudElements.push_back(new Entitie(this, "key_model", Block::WHITE, "basic-nolight", Vector3(0.15, 0.048, 0.065), 0.08));
	hudElements.push_back(new Entitie(this, "enemy_text", Block::WHITE, "basic-nolight", Vector3(0.15, 0.058, 0.08), 0.002));

	//OBJECTIVE SCORE	
	hudElements.push_back(new Entitie(this, "objective_text", Block::WHITE, "basic-nolight", Vector3(0.15, 0.058, 0.0), 0.002));
